<?php
  ...
  if($setting["dompart"] > 0 && strlen($json->dom) > $setting["dompart"]) $domExtra = "\n\nView full dom on the report page or change this setting on /settings";

  if(isset($json->shared)) {
    if($setting["secretkey"] != $json->shared) { echo 0; die(); }
    $userIP = $json->ip;
  }
  ...
  if($setting["filter-save"] == 0 || $setting["filter-alert"] == 0) {
    ...
    array(":cookies" => $json->cookies, ":dom" => $json->dom, ":origin" => $json->origin, ":referer" => $json->referrer, ":uri" => $json->uri, ":userAgent" => $json->{'user-agent'}, ":ip" => $userIP));

    if(isset($check["id"])) {
      if($setting["filter-save"] == 0 && $setting["filter-alert"] == 0) die();
      $doubleReport = true;
    }
  }
  ...
  if( ($doubleReport && $setting["filter-save"] == 1) || (!$doubleReport) ) {
    ...
    array(":cookies" => $json->cookies, ":dom" => $json->dom, ":origin" => $json->origin, ":referer" => $json->referrer, ":screenshot" => $imageName, ":uri" => $json->uri, ":userAgent" => $json->{'user-agent'}, ":ip" => $userIP, ":time" => time()));
  }
  ...
  if( ($doubleReport && $setting["filter-alert"] == 1) || (!$doubleReport) ) {
    $htmlTemplate = str_replace(
      ...
      [$id, $domain, htmlspecialchars($json->uri), $userIP, htmlspecialchars($json->referrer), htmlspecialchars($json->{'user-agent'}), htmlspecialchars($json->cookies), htmlspecialchars(substr($json->dom, 0, $setting["dompart"])) . $domExtra, $imageUrl, htmlspecialchars($json->origin), date("F j Y, g:i a")],
