<?php
  ...
  class Route {
    ...
    private function templateHtml($file) {
      $html = str_replace(['{{template}}', '{{menu}}', '{{name}}', '{{domain}}'], [$this->getFile($file), $this->getFile('site/menu'), ucwords(htmlspecialchars($file)), htmlspecialchars($_SERVER['HTTP_HOST'])], $this->getFile('site/main'));
